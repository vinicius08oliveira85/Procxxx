  ...
  return (
    <>
      <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        <div
          className="absolute right-[-5%] top-[-10%] h-[40vw] w-[40vw] rounded-full bg-[#2563eb]/10 blur-[120px]"
          aria-hidden
        />
        <div
          className="absolute bottom-[-10%] left-[-5%] h-[30vw] w-[30vw] rounded-full bg-[#571bc1]/20 blur-[100px]"
          aria-hidden
        />
      </div>

      <header className="fixed top-0 z-50 flex h-16 w-full items-center justify-between border-b border-[#434655]/15 bg-[#1c1b1b]/80 px-4 backdrop-blur-xl sm:px-8">
        <div className="flex min-w-0 items-center gap-4 sm:gap-6">
          ...
          <nav className="ml-2 hidden items-center gap-6 md:flex">
            <button
              type="button"
              onClick={onGoUpload}
              className="text-[10px] font-medium uppercase tracking-widest text-[#8d90a0] transition-colors hover:text-white"
            >
              Visão Geral
            </button>
            <button
      ...
      <footer className="fixed bottom-0 z-50 w-full border-t border-[#434655]/10 bg-[#131313]/40 px-4 py-5 backdrop-blur-2xl sm:px-8">
          ...
          <div className="flex flex-wrap items-center justify-end gap-3">
            {showNext && (
              <button
                type="button"
                onClick={onNext}
                className="min-h-[44px] rounded-full border border-[#434655]/30 px-6 py-3 font-medium text-[#e5e2e1] transition-all hover:bg-white/5 sm:px-8"
              >
                Próximo
              </button>
            )}
            <button
              ...
              onClick={onExecute}
              className={cn(
                'flex min-h-[44px] items-center justify-center gap-2 rounded-full px-8 py-4 font-bold text-white shadow-[0_10px_25px_rgba(37,99,235,0.35)] transition-all active:scale-95 sm:px-10',
                executeDisabled
                  ? 'cursor-not-allowed bg-zinc-600 opacity-50 shadow-none'
                  : 'bg-gradient-to-r from-[#2563eb] to-[#571bc1] hover:scale-[1.02]'
              )}
            >
              {executeLoading ? (
                <>
  ...
  return (
    <div className="mb-4 flex w-full max-w-2xl items-center justify-between">
      <div className="flex flex-col items-center gap-2 opacity-60">
        ...
        <span className="font-jakarta text-[10px] font-medium uppercase tracking-widest text-[#8d90a0]">
          1. Upload
        </span>
      </div>
